...
| CLI 命令 | 描述 | 使用场景 |
| :--- | :--- | :--- |
| `claude` | 在当前目录启动交互式会话。 | 开始一个新任务。 |
| `claude "query"` | 启动新会话并直接执行初始提示词。 | 快速提问，需要立即获得答案。 |
| `claude -p "query"` | **一次性执行**。执行完查询后自动退出，适合脚本或自动化。 | 在 CI/CD 流程中集成，或快速获取单一结果。 |
| `claude -c` / `--continue` | 恢复最近的会话。 | 继续昨天未完成的工作。 |
| `claude -r <session-id>` | 通过 ID 恢复一个特定的历史会话。 | 精确切换到之前某个特定任务。 |
| `claude update` | 将 Claude Code 更新到最新版本。 | 确保使用最新的功能和修复。 |
| `claude --version` | 查看当前安装的版本号。 | 检查版本或排查问题。 |
| `claude config` | 管理 Claude Code 的配置。 | 设置全局偏好。 |
| `claude mcp` | 管理 MCP (Model Context Protocol) 服务器的连接。 | 连接数据库、API 等外部工具。 |
| `claude --dangerously-skip-permissions` | 在当前目录启动交互式会话。(dangerously) | 开始一个新任务。(dangerously) |
...
其中我对setting.json文件做了修改，`defaultMode` 设置成了 `bypassPermissions`。修改之后为：
...
国内使用Claude自带模型比较麻烦，我是注册了Claude之后，然后替换成了Deepseek的 API Key。

只需要在 `.bashrc` 或者 `/etc/profile` 文件中新增对应的环境变量就行。
...
| 命令 | 描述 | 使用场景 |
| :--- | :--- | :--- |
| `/init` | 扫描项目并生成 `CLAUDE.md`，让 AI 了解项目技术栈和规范。 | 首次使用或项目结构重大变化时运行。 |
| `/help` | 查看所有可用命令的帮助信息。 | 忘记命令或需要快速查阅时。 |
| `/memory` | 直接编辑 `CLAUDE.md` 文件，添加永久记忆规则。 | 添加项目级编码规范或个人偏好。 |
| `/config` | 调整权限、行为偏好等配置。 | 修改模型、权限、默认模式等。 |
| `/add-dir` | 添加额外的项目目录（适合 monorepo）。 | 需要让 AI 同时理解多个子项目。 |

对话控制

| 命令 | 描述 | 使用场景 |
| :--- | :--- | :--- |
| `/compact` | 压缩长对话历史，释放上下文窗口。 | 对话太长导致 AI 变慢或“遗忘”。 |
| `/clear` | 完全清除当前对话历史，全新开始。 | 对话跑偏，想从头再来。 |
| `/rewind` | 对话回滚，代码和对话状态一起回到之前的节点。 | 某次修改错误，想回到先前状态。 |
| `/btw` | 开一个不打断主任务的“侧边聊天”窗口。 | 想问一个无关小问题，但不想破坏主线。 |
| `/export` | 将当前对话导出为 Markdown 文件。 | 保存重要讨论，用于文档或分享。 |

开发辅助

| 命令 | 描述 | 使用场景 |
| :--- | :--- | :--- |
| `/plan` | 进入规划模式，AI 只做方案设计，不修改代码。 | 复杂任务先确认方案再动手。 |
| `/model` | 在会话中切换 AI 模型。 | 如用 Opus 规划，Sonnet 编码。 |
| `/review` | 让 AI 对代码进行审查，提供改进建议。 | 提交前检查代码质量。 |
| `/simplify` | 对代码进行三重审查，帮助简化复杂代码。 | 重构老旧或复杂的函数。 |
| `/batch` | 批量处理多个文件的修改。 | 统一格式化、批量重命名等。 |

监控与诊断

| 命令 | 描述 | 使用场景 |
| :--- | :--- | :--- |
| `/cost` | 查看当前会话的费用和 Token 消耗。 | 控制预算，避免意外超支。 |
| `/doctor` | 诊断系统配置和连接问题。 | Claude Code 出奇奇怪怪的报错时。 |
| `/status` | 查看当前会话和代理状态。 | 确认当前模式、模型、连接情况。 |
| `/stats` | 查看每日 Token 用量的可视化趋势。 | 回顾一段时间的使用量。 |
| `/insights` | 生成月度使用分析报告。 | 深入了解 Token 都用在哪了。 |

自动化与高级功能

| 命令 | 描述 | 使用场景 |
| :--- | :--- | :--- |
| `/powerup` | 官方交互式教程。 | 系统化学习所有功能。 |
| `/loop` | 设置定时任务，让 AI 重复执行某些操作。 | 周期性运行测试或清理任务。 |
| `/remote-control` | 允许从手机等设备远程操控 Claude Code。 | 外出时远程控制编码。 |
| `/tasks` | 查看和管理后台运行的异步任务。 | 查看长时间运行的 AI 任务进度。 |
| `/skills` | 列出所有可用的 Skill 扩展。 | 查看已安装或可用的技能插件。 |
| `/hook` | 配置 Git 钩子等自动化工作流。 | 在 commit、push 时自动触发 AI 检查。 |
| `/vim` | 切换到 Vim 编辑模式。 | 习惯 Vim 键位的开发者。 |

#### 第三方工具

可以使用三方路由器 `claude-code-router` 来切换deepseek model。
...
我平常使用Kimi的模型会更多一些，然后看到官网有提供Kimi Code，就尝试一番。

使用下来的感觉是和Claude Code没有太多区别，大部分命令差不多，但是还是有一些差别，比如配置中kimi使用 `yolo` 来表示`全部自动批准`，和Claude Dangerous模式类似。

我订阅的是49一个月的套餐，其中有 `本周用量` 和 `频限明细` 的限制，这个量很少。我在项目中 `/init` 以及添加一个feature，`频限明细`就达到限额了，需要5小时之后重置。太少了。
...
这里应该是我使用过的最便宜的模型了，但是`便宜`这个词需要讨论一下。

按照Token的价格，确实是最便宜的，但是使用模型的最终用途是完成一些事项，比如编程中是想完成某些feature，这个时候 `价钱` 应该就是 `总价`/ `Feature数量`。如果按照这样算的话，其实Deepseek可能没有便宜那么多。使用下来往往一个feature，DeepSeek消耗的token量更多，并且因为消耗的token量多，还会导致 `This model's maximum context length is 131072 tokens` 超过上下文。然后人工投入的精力也会稍多一些。

## moonshot

因为kimi有活动，充值多少钱，会补一些钱，所以我充值了之后，使用的就是它家多一些。体感要比Deepseek好很多，而且往往能使用比deepseek少的token完成对应的feature。少有遇到超过上下文限制的问题。
...
我在卧室使用的是Mac Mini M4搭配显示器，我在客厅分别放置了我以前使用的Mac Mini Intel，以及一台Windows电脑。我平常使用 M4 Mac Mini连接另外两台电脑，使用的是 [parsec](https://parsec.app/) 和 `Windows APP` ，这样电脑风扇的声音就不会吵到我。玩游戏之类的延迟和画质也能接受。
...
Claude code 配合本地的模型不太好用。首先就是各种参数和兼容性的问题，我觉得这个是主要问题吧！
